  ...
  private void switchGetMoveList(String input) {
    ...
    if (myBoard[piece.getRow()][piece.getCol()] != null) {
      if (!(myBoard[piece.getRow()][piece.getCol()] instanceof Pawn)) {
        // not Pawn
        myDisplay.printMove(
            input, myBoard[piece.getRow()][piece.getCol()].getValidMoveList(myBoard));
      } else {
        // Pawn
        Pawn p = (Pawn) myBoard[piece.getRow()][piece.getCol()];
        myDisplay.printMove(input, p.getValidMoveList(myBoard, counter));
      }
    } else myDisplay.printMoveFail();
  }
    ...
    if (myBoard[piece2.getRow()][piece2.getCol()] != null) {
      ...
      if (myBoard[piece2.getRow()][piece2.getCol()].isWhite == isWhiteTurn) {
        ...
        if (!(myBoard[piece2.getRow()][piece2.getCol()] instanceof Pawn)) {
          ...
          moveOK = p.move(newPosition, myBoard, counter);
        }
        if (moveOK) {
          ...
          myBoard[newPosition.getRow()][newPosition.getCol()] =
              myBoard[piece2.getRow()][piece2.getCol()];
