...
public abstract class Piece {
  // Constants
  /**
   * protected position value array that child classes' constructor can use to set its starting
   * position will be filled with actual data...
   */
  protected final Position[] START_POSITION = null;

  // Fields
  /**
   * String type: Type of the piece, this will be assigned at the initialization ex) Queen, Rook...
   *
   * <p>boolean color: White - true, black - false
   *
   * <p>Position position: Position of the piece, this will be assigned at the initialization with
  ...
  /**
   * move the position of this piece - common method for the all pieces this move method has no
   * safety codes, so you must call isValidMove() for validity check if other player's piece is
   * occupying the destination, your piece will capture
   *
  ...
  /**
   * check the destination point is available to move this method checks only the grid safety(is
   * this piece on the board), so you should override and use this method inside of the pieces'
   * isValidMove()
   *
  ...
  protected boolean isValidMove(Position newPosition, Piece[][] board) {
    ...
    if (newPosition.getRow() >= 0
        ...
        && newPosition.getCol() <= 7) result = true;
